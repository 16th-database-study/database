...
※ 주의사항:
B-Tree, B+Tree는 서로 구분하지 않고 그냥 B-Tree로 작성하였습니다.
...
인덱스 키 값에 따라 그 값에 따라 저장될 리프 노드의 위치가 결정이 된다. B-Tree의 키 값이 변경되는 경우엔 인덱스 상의 키 값을 변경하는 방식으로 동작하지 않는다.

모든 연관있는 것에 대해 키 값을 삭제한 뒤 다시 키 값을 추가하는 방식으로 동작한다. 이러한 처리는 모두 체인지 버퍼를 통해서 지연 처리가 된다.
...
![img.png](image/PaikMyeongGyu/clustering.png)
...
![img.png](image/b_tree_index_struct.png)
...
만약 key키가 16 바이트이고 주소의 바이트는 12바이트라 하면 대략 585개를 페이지마다 담을 수 있다.

그러면 위의 key, 주소 형태에서 585 * 585 레코드까진 깊이 2, 585 * 585 * 585(약 2억건)까진 깊이 3으로 처리를 할 수 있다.

반면 이진트리를 사용하면 1024 레코드까지 깊이 10, 1024 * 1024까진 깊이 20 등으로 레코드가 늘어남에 따라 깊이가 급격하게 커진다.
...
[초기 ISAM 트리 구조]
![img.png](image/PaikMyeongGyu/isam2.png)

[추가 삽입이 일어났을 때 ISAM 트리 구조]
![img_1.png](image/PaikMyeongGyu/isam1.png)
...
이 체크 과정에서 MySQL에선 유니크 인덱스에서 중복된 값을 체크하기 위해 읽기 잠금을 사용하고, 쓰기를 할 댄 쓰기 잠금을 사용해서 이 과정에서의
